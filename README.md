...
確認清單出現磁碟名稱與容量（50 GiB）後，點擊 **「下一步：認證 >」**。
...
|------|---------|
| 鑰匙對認證 | **停用** |
| 密碼 | 自訂密碼（需符合：大寫、小寫、數字、特殊符號各至少 1 個，8～72 字元） |
...
# ── Clone bootcamp 教材（寫入 image） ────────────────────────
WORKDIR /workspace
RUN git clone --depth 1 \
        https://github.com/openhackathons-org/AI-Powered-Physics-Bootcamp.git
...
WORKDIR /workspace/AI-Powered-Physics-Bootcamp
...
```bash
cat > ~/physicsnemo-workshop/docker-compose.yml << 'COMPOSE_EOF'
services:
  physicsnemo-lab:
    image: physicsnemo-bootcamp:25.06
    build:
      context: .
      dockerfile: Dockerfile
      args:
        PHYSICSNEMO_VERSION: "25.06"
    container_name: physicsnemo-bootcamp
    ...
    # 持久化工作成果（容器刪除後資料保留）
    volumes:
      - tutorial_results:/workspace/AI-Powered-Physics-Bootcamp/tutorial/results
      - challenge_results:/workspace/AI-Powered-Physics-Bootcamp/challenge/results
...
```bash
cat ~/physicsnemo-workshop/docker-compose.yml
```
...
| `git clone` | 將 bootcamp 教材下載並固定在 image 中 |
...
預期輸出（`WARNING` 為正常資訊訊息，非錯誤）：

```
WARNING: This output is designed for human readability. ...
physicsnemo-bootcamp   25.06   <IMAGE_ID>   ...   17GB
```
...
預期輸出（`STATUS` 顯示 `healthy` 代表 JupyterLab 已就緒）：

```
NAME                   IMAGE                        COMMAND    SERVICE           CREATED        STATUS                  PORTS
physicsnemo-bootcamp   physicsnemo-bootcamp:25.06   "..."      physicsnemo-lab   X seconds ago  Up X seconds (healthy)  6006/tcp, 0.0.0.0:8888-8889->8888-8889/tcp
```

### 10-3. 查看啟動日誌

```bash
docker compose logs -f physicsnemo-lab
```
...
...
[W] All authentication is disabled.  Anyone who can connect to this server will be able to run code.
...
[I] Serving notebooks from local directory: /workspace/AI-Powered-Physics-Bootcamp
[I] Jupyter Server 2.15.0 is running at:
[I] http://hostname:8888/lab
...
# ── 容器管理 ──────────────────────────────────────────────
# 啟動（背景）
docker compose -f ~/physicsnemo-workshop/docker-compose.yml up -d

# 停止（保留資料 volume）
docker compose -f ~/physicsnemo-workshop/docker-compose.yml down

# 停止並清除 volume（謹慎！學員成果將消失）
docker compose -f ~/physicsnemo-workshop/docker-compose.yml down -v

# ── 狀態查看 ──────────────────────────────────────────────
docker compose -f ~/physicsnemo-workshop/docker-compose.yml ps
docker compose -f ~/physicsnemo-workshop/docker-compose.yml logs -f
...
# ── 重新 build image ──────────────────────────────────────
docker compose -f ~/physicsnemo-workshop/docker-compose.yml build --no-cache
docker compose -f ~/physicsnemo-workshop/docker-compose.yml up -d
```
...
確認 `docker-compose.yml` 中的以下設定存在且生效：
...
**切換版本**：修改 `docker-compose.yml` 中的 `image` 與 `PHYSICSNEMO_VERSION` 後重新 build：

```yaml
image: physicsnemo-bootcamp:25.11
build:
  args:
    PHYSICSNEMO_VERSION: "25.11"
```
